...
public class ApiImpl implements Api {
    ...
    @Override
    public void getOpportunity(String oppName, final OpportunityCallback opportunityCallback) {
        try {
            String url = OPPORTUNITY + URLEncoder.encode(oppName, "utf-8").replace("+", "%20");
            restApi.get(url, new StringCallback() {
                @Override
                public void onCompleted(String response) {

                    Opportunity opp = gson.fromJson(response, Opportunity.class);
                    opportunityCallback.onCompleted(opp);
                }
